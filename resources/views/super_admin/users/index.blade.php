...
<head>
    ...
    <style>
        ...
        body {
            ...
            background-color: #f3f4f6; /* Pastikan body memiliki background */
        }
...
<body class="bg-gray-100 font-sans leading-normal tracking-normal min-h-screen flex flex-col md:flex-row">
    ...
    <div class="flex-grow p-6 md:p-10 main-content">
        {{-- Memberi nama kelas spesifik untuk header utama aplikasi --}}
        <header class="bg-white rounded-xl shadow-lg flex items-center justify-between mb-8 main-app-header">
            <div class="flex items-center">
                ...
                <h1 class="text-4xl font-extrabold text-gray-800">Kelola User</h1>
            </div>
            {{-- Bagian welcome avatar desktop (jika ada) --}}
            <div class="desktop-welcome hidden md:flex items-center">
                <span class="text-gray-700 font-semibold text-lg mr-2">Selamat Datang, Super Admin!</span>
                <div class="desktop-avatar">SA</div>
            </div>
        </header>
        ...
        <div class="bg-white p-8 rounded-xl shadow-lg col-span-full card-hover-effect">
            {{-- Header internal di dalam card "Daftar User" --}}
            <header class="flex items-center justify-between pb-4 border-b border-gray-200 mb-6">
                <h2 class="text-2xl font-semibold text-gray-800 md:text-2xl">Daftar User</h2>
                <a href="{{ route('super-admin.users.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-150 ease-in-out shadow-md md:py-2 md:px-4 md:text-base">
                    Tambah User Baru
                </a>
            </header>

            @if ($users->isEmpty())
                <p class="text-gray-600 text-center py-4">Belum ada user terdaftar.</p>
            @else
                    ...
                    <table class="min-w-full bg-white">
                        <thead>
                            <tr>
                                ...
                                <th class="py-3 px-4 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Nama</th>
                                <th class="py-3 px-4 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Email</th>
                                <th class="py-3 px-4 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Role</th>
                        ...
                        <tbody>
                            @foreach ($users as $user)
                                <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                                    ...
                                    <td class="py-3 px-4 border-b border-gray-200 text-sm text-gray-700">{{ optional($user->role)->name ?? 'Tidak Ada Role' }}</td>
                                    <td class="py-3 px-4 border-b border-gray-200 whitespace-nowrap">
                                        <a href="{{ route('super-admin.users.edit', $user->id) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-1.5 px-3 rounded-md text-xs mr-2 transition duration-150 ease-in-out">Edit</a>
                                        <form action="{{ route('super-admin.users.destroy', $user->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus user ini?');">
                                            @csrf
                                            @method('DELETE')
