...
<style>
    ...
    /* ** NEW: Adjusting for scrollbar compensation ** */
    html.overflow-hidden-active {
        overflow: hidden;
    ...
    @media (max-width: 767px) { /* Tailwind's 'md' breakpoint is 768px, so less than that is mobile */
        ...
        .main-app-header h1 {
            font-size: 1.25rem; /* Larger font size for main mobile header title */
            font-weight: bold;
        ...
        .main-content {
            ...
            padding-top: 4.5rem; /* This is the key change: 3.5rem (approx header height) + 1rem (desired gap) */
        ...
        .bg-red-100.border.border-red-400.text-red-700 ul li {
            font-size: 0.75rem; /* Smaller font for list items in alert */
        }
    }
...
<body class="bg-gray-100 font-sans leading-normal tracking-normal min-h-screen flex flex-col md:flex-row">
    ...
    <div class="flex-grow p-6 md:p-10 main-content">
        ...
        <header class="bg-white rounded-xl shadow-lg flex items-center justify-between mb-8 main-app-header">
            <div class="flex items-center">
                ...
                <h1 class="text-4xl font-extrabold text-gray-800">Edit User</h1>
            </div>
        ...
        <div class="bg-white p-8 rounded-xl shadow-lg card-hover-effect">
            {{-- REMOVED mb-6 FROM THIS HEADER, now controlled by .main-content .bg-white.p-8 header in CSS --}}
            ...
            <form action="{{ route('super-admin.users.update', $user->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $user->name) }}" class="shadow appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out" required>
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $user->email) }}" class="shadow appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out" required>
                    @error('email')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-4">
                    ...
                    <input type="password" id="password" name="password" class="shadow appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out">
                    @error('password')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-4">
                    ...
                    <input type="password" id="password_confirmation" name="password_confirmation" class="shadow appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out">
                </div>
                <div class="mb-4">
                    ...
                    <select id="role_id" name="role_id" class="shadow appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out" required>
                        ...
                        @foreach ($roles as $role)
                            {{-- Tambahkan data-slug ke opsi role --}}
                ...
                <div class="mb-4" id="stasi_field_container">
                    ...
                    <select name="stasi_id" id="stasi_id" class="shadow appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out">
                        ...
                        @foreach ($stasis as $stasi)
                            <option value="{{ $stasi->id }}" {{ old('stasi_id', $user->stasi_id) == $stasi->id ? 'selected' : '' }}>
                                {{ $stasi->nama }}
                            </option>
                        @endforeach
                    </select>
    ...
    <script>
        function toggleSidebar() {
            ...
            html.classList.toggle('overflow-hidden-active'); // Toggle class on HTML
        }
        ...
        window.addEventListener('resize', () => {
            ...
            if (window.innerWidth >= 768) { // Tailwind's 'md' breakpoint
                ...
                html.classList.remove('overflow-hidden-active'); // Remove class on HTML
            }
        });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function toggleStasiField() {
                ...
                const selectedRoleSlug = selectedOption.dataset.slug; // Ambil slug dari data-slug
                ...
                if (selectedRoleSlug === 'bendahara-stasi' || selectedRoleSlug === 'ketua-stasi') {
                    ...
                    stasiSelect.removeAttribute('disabled'); // Aktifkan dropdown
                    stasiSelect.setAttribute('required', 'required'); // Make it required for these roles
                } else {
                    stasiFieldContainer.style.display = 'none'; // Sembunyikan container
                    stasiSelect.setAttribute('disabled', 'disabled'); // Nonaktifkan dropdown
                    stasiSelect.removeAttribute('required'); // Remove required
                    stasiSelect.value = ''; // Opsional: reset nilai stasi
                }
            }

            // Panggil fungsi saat halaman dimuat (untuk kasus nilai user saat ini)
            toggleStasiField();

            // Panggil fungsi saat pilihan role berubah
